<template>
    <file-icon :icon="item.type" :title="item.basename" data-selectable>
            <span :class="{'selectable' : selectMode}">
                {{ listview ? item.basename : title_shorten(item.basename) }}
            </span>

        <span v-show="listview" class="vuefinder-file-size is-hidden-mobile">
                {{ item.type != 'folder' ? fileSizeIEC(item.size) : '' }}
            </span>

        <span v-show="listview" class="vuefinder-file-time is-hidden-mobile">
                {{ time(item.timestamp) }}
            </span>
    </file-icon>
</template>
