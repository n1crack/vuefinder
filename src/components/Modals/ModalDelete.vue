<template>
    <modal @close="$emit('close')">
            ...
            <ul class="delete-list">
                <li v-for="item in data" :key="item.path">
                    <div class="delete-list-item">
                        <span class="file_path">{{item.dirname?'/':''}}{{item.dirname}}/</span><span class="file_name">{{item.basename}}</span>
                    </div>
        ...
        <div class="vuefinder-modal-footer">
            <button class="vuefinder-button" @click="remove">Yes, delete!</button>
            <button class="vuefinder-button" @click="$emit('close')">Cancel</button>
            <div style="color:crimson">
                <span style="font-weight:bold">Caution:</span>
                You <span style="font-weight:bold">cannot</span> undo this action!
            </div>
        </div>
    </modal>
</template>
