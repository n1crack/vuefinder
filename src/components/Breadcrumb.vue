<template>
  <div class="flex p-1.5 bg-neutral-100 dark:bg-gray-800 border-t border-b border-neutral-300 dark:border-gray-900 items-center select-none ">
    <svg @click="!breadcrumb.length || emitter.emit('vf-fetch-index', {adapter: data.adapter, item:breadcrumb[breadcrumb.length-2]} )" xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="flex bg-white dark:bg-gray-700 items-center rounded p-1 ml-2 w-full">
      <svg @click="emitter.emit('vf-fetch-index', {adapter: data.adapter})"
      ...
      <div class="flex leading-5">
        <div v-for="(item, index) in breadcrumb" :key="index">
          <span class="text-neutral-300 dark:text-neutral-800 mx-0.5">/</span>
          <span class="px-1.5 py-1 text-slate-700 dark:text-slate-200 hover:bg-neutral-100 dark:hover:bg-gray-800 rounded cursor-pointer" :title="item.basename" @click="emitter.emit('vf-fetch-index', {adapter: data.adapter, item})">{{ item.name }}</span>
        </div>
      </div>
    </div>
  </div>
