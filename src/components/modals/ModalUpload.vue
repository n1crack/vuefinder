  ...
  <v-f-modal-layout>
    <div class="sm:flex sm:items-start">
      ...
      <div class="mt-3 text-center sm:mt-0 sm:text-left">
        <h3 class="text-lg leading-6 sm:ml-4 font-medium text-gray-900 dark:text-gray-400" id="modal-title">Upload files</h3>
        <div class="mt-2 sm:ml-4">
          <div class="text-gray-500 mb-1">
            <div v-for="file in queue"><div :id="file.id"> {{ file.name }} ( {{ file.size }}) <b>{{ file.percent }}</b></div></div>

            <div class="py-2" v-if="!queue.length"> No files selected!</div>
          </div>
          <div class="text-gray-500" :ref="(el) => container = el">
              <button :ref="(el) => pickFiles = el"  type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Select Files</button>
          </div>
        </div>
      </div>
    </div>
