...
export function useStorage(key: string) {
    ...
    function setStore(name: string, value: any) {
        storage[name] = value;
    }
    ...
    function clearStore() {
        Object.keys(storage).forEach(k => removeStore(k));
    }

    const getStore = <T = any>(name: string, defaultValue: T | null = null): T | null => {
        return name in storage ? storage[name] : defaultValue;
    };
