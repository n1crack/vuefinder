import {
  ...
  createBlock as V,
  resolveDynamicComponent as Tn,
  withCtx as Q,
  createVNode as I,
  Fragment as de,
  renderList as fe,
  createTextVNode as oe,
  withDirectives as me,
  vModelText as ft,
  resolveComponent as In,
  normalizeClass as j,
  vModelCheckbox as Zt,
  customRef as go,
  onUnmounted as xe,
  Teleport as Et,
  normalizeStyle as He,
  isRef as wo,
  onBeforeUnmount as yo,
  vModelSelect as qt,
  vModelRadio as Kt,
  mergeProps as Te,
  toHandlers as We,
  vShow as ze,
  normalizeProps as rt,
  guardReactiveProps as dt,
  TransitionGroup as bo,
  onUpdated as xo,
  mergeModels as ko,
  useModel as On,
  provide as $o,
...
function ee() {
  const t = Jt(Rn);
  if (!t) throw new Error('ServiceContainer was not provided');
  return t;
}
function To(t) {
  ...
  return {
    getStore: (c, _ = null) => (c in n ? n[c] : _),
    setStore: i,
    removeStore: r,
    clearStore: a,
  };
...
function Oo(t, e, n, l) {
        ...
        .then((b) => {
          ((a.value = b),
            ...
            Object.values(l).length > 1 &&
              (n.emit('vf-toast-push', { label: 'The language is set to ' + d }),
        ...
        .catch((b) => {
          h
            ? (n.emit('vf-toast-push', {
              ...
              c(h, null))
            : (console.error(b),
              n.emit('vf-toast-push', { label: 'Locale cannot be loaded!', type: 'error' }));
        });
    };
  (ue(f, (d) => {
    c(d);
  ...
  function v(d, ...h) {
    return a.value && Object.prototype.hasOwnProperty.call(a.value, d)
      ? _(a.value[d] || d, ...h)
      : _(d, ...h);
  }
...
function en(t, e, n, l, i) {
  return (
    ...
    (l = 1024),
    (i = (n(t) / n(l)) | 0),
    (t / e.pow(l, i)).toFixed(0) + ' ' + (i ? 'KMGTPEZY'[--i] + 'iB' : 'B')
  );
}
function Vn(t, e, n, l, i) {
  return (
    ...
    (l = 1e3),
    (i = (n(t) / n(l)) | 0),
    (t / e.pow(l, i)).toFixed(0) + ' ' + (i ? 'KMGTPEZY'[--i] + 'B' : 'B')
  );
}
function Vo(t) {
  if (typeof t == 'number') return t;
  const e = { k: 1, m: 2, g: 3, t: 4 },
    l = /(\d+(?:\.\d+)?)\s?(k|m|g|t)?b?/i.exec(t);
  if (!l) return 0;
  const i = parseFloat(l[1] || '0'),
    r = (l[2] || '').toLowerCase(),
    a = e[r] ?? 0;
...
const Ho = () => {
    const t = Ce(''),
      ...
      n = Ce(!1),
      l = Ce([]),
      i = Ce({ active: !1, column: '', order: '' }),
      r = Ce({
        kind: 'all',
        showHidden: !1,
      }),
      a = Ce(/* @__PURE__ */ new Set()),
      f = Ce({
        type: 'copy',
        path: '',
      ...
      h = Ce(-1),
      b = qe([t], (T) => {
        const P = (T || 'local://').trim(),
          N = P.indexOf('://'),
          K = N >= 0 ? P.slice(0, N) : '',
          ke = (N >= 0 ? P.slice(N + 3) : P).split('/').filter(Boolean);
        let $e = '';
        const Ut = ke.map(
          (Ae) => (
            ($e = $e ? `${$e}/${Ae}` : Ae),
      ...
      D = qe([l, i, r], (T, P, N) => {
        let K = T;
        (N.kind === 'files'
          ? (K = K.filter((Ae) => Ae.type === 'file'))
          : N.kind === 'folders' && (K = K.filter((Ae) => Ae.type === 'dir')),
          N.showHidden || (K = K.filter((Ae) => !Ae.basename.startsWith('.'))));
        const { active: ge, column: ke, order: $e } = P;
        if (!ge || !ke) return K;
        const Ut = $e === 'asc' ? 1 : -1;
        return K.slice().sort((Ae, po) => zo(Ae[ke], po[ke]) * Ut);
      }),
      g = qe([l, a], (T, P) => (P.size === 0 ? [] : T.filter((N) => P.has(N.path)))),
      p = (T, P) => {
        const N = t.get();
        if ((P ?? !0) && N !== T) {
          const K = d.get(),
            ge = h.get();
          (ge < K.length - 1 && K.splice(ge + 1),
            K.length === 0 && N && K.push(N),
            K.push(T),
            d.set([...K]),
            h.set(K.length - 1));
        }
      ...
      pe = (T = 'multiple', P) => {
        if (T === 'single') {
          ...
          P?.selectionFilterType ||
          (P?.selectionFilterMimeIncludes && P.selectionFilterMimeIncludes.length > 0)
        ) {
          const N = l
            .get()
            .filter((K) => {
              const ge = P.selectionFilterType,
                ke = P.selectionFilterMimeIncludes;
              return (ge === 'files' && K.type === 'dir') || (ge === 'dirs' && K.type === 'file')
                ? !1
                : ke && Array.isArray(ke) && ke.length > 0 && K.type !== 'dir'
                  ? K.mime_type
                    ? ke.some(($e) => K.mime_type?.startsWith($e))
                    : !1
                  : !0;
            })
            .map((K) => K.path);
      ...
      F = (T) => qe([f], (P) => P.type === 'cut' && Array.from(P.items).some((N) => N.path === T)),
      A = (T) => qe([f], (P) => P.type === 'copy' && Array.from(P.items).some((N) => N.path === T)),
      H = (T) => {
        const P = F(T);
        return W(P).value ?? !1;
      },
...
class No {
  ...
  constructor(e, n = {}) {
    ((this.adapter = e),
      (this.onBeforeOpen = n.onBeforeOpen),
      (this.onAfterOpen = n.onAfterOpen),
      (this.queryClient =
        n.queryClient ||
        new Do({
          defaultOptions: {
...
const Ko = (t, e) => {
  ...
  Wo = ['data-theme'],
  jo = { class: 'vuefinder__modal-layout__container' },
  Go = { class: 'vuefinder__modal-layout__content' },
  qo = {
    key: 0,
  ...
  Qo = { class: 'vuefinder__modal-drag-message' },
  Ee = /* @__PURE__ */ X({
    __name: 'ModalLayout',
    props: {
    ...
    setup(t) {
      ...
      const i = (r) => {
        r.target.classList.contains('vuefinder__modal-layout__wrapper') &&
              ...
              o(
                ...
                [
                  o(
                    ...
                    [
                      o('div', Go, [De(r.$slots, 'default')]),
                      r.$slots.buttons ? (u(), w('div', qo, [De(r.$slots, 'buttons')])) : M('', !0),
                    ],
                    512
                  ),
                ],
              ...
              ? (u(),
                w('div', Yo, [
                  o(
                    'div',
                    Qo,
                    y(l.dragOverlayText || 'Drag and drop the files/folders to here.'),
                    1
                  ),
                ]))
              : M('', !0),
  ...
  Xo = { class: 'vuefinder__modal-header' },
  Jo = { class: 'vuefinder__modal-header__icon-container' },
  Zo = {
  ...
  Me = /* @__PURE__ */ X({
    ...
    setup(t) {
      return (e, n) => (
        u(),
        w('div', Xo, [
          o('div', Jo, [(u(), V(Tn(t.icon), { class: 'vuefinder__modal-header__icon' }))]),
...
const Pn = { render: ts },
  ns = { class: 'vuefinder__about-modal__content' },
  os = { class: 'vuefinder__about-modal__main' },
  ss = { class: 'vuefinder__about-modal__tab-content' },
  ls = { class: 'vuefinder__about-modal__lead' },
  is = { class: 'vuefinder__about-modal__description' },
  as = { class: 'vuefinder__about-modal__links' },
  rs = {
  ...
  ds = { class: 'vuefinder__about-modal__meta' },
  cs = { class: 'vuefinder__about-modal__meta-item' },
  us = { class: 'vuefinder__about-modal__meta-label' },
  vs = { class: 'vuefinder__about-modal__meta-value' },
  fs = { class: 'vuefinder__about-modal__meta-item' },
  _s = { class: 'vuefinder__about-modal__meta-label' },
  Bn = /* @__PURE__ */ X({
    __name: 'ModalAbout',
    setup(t) {
      const e = ee(),
              ...
              o('div', os, [
                o('div', ss, [
                  o(
                    'div',
                    ls,
                    y(s(n)('A modern, customizable file manager component built for Vue.')),
                    1
                  ),
...
const zn = { render: ps },
  hs = { class: 'vuefinder__delete-modal__content' },
  gs = { class: 'vuefinder__delete-modal__form' },
  ws = { class: 'vuefinder__delete-modal__description' },
  ys = { class: 'vuefinder__delete-modal__files vf-scrollbar' },
  bs = { class: 'vuefinder__delete-modal__file' },
  xs = {
    key: 0,
  ...
  $s = { class: 'vuefinder__delete-modal__file-name' },
  Cs = { class: 'vuefinder__delete-modal__warning' },
  Mt = /* @__PURE__ */ X({
    __name: 'ModalDelete',
    setup(t) {
      const e = ee(),
        ...
        l = e.fs,
        i = W(l.path),
        r = E(e.modal.data.items),
        a = E(''),
        f = () => {
          (console.log(r.value.map(({ path: c, type: _ }) => ({ path: c, type: _ }))),
            r.value.length &&
              e.adapter
                .delete({
                  path: i.value.path,
...
const Hn = { render: Fs },
  Ds = { class: 'vuefinder__rename-modal__content' },
  Es = { class: 'vuefinder__rename-modal__item' },
  As = { class: 'vuefinder__rename-modal__item-info' },
  Ms = {
    key: 0,
  ...
  Is = { class: 'vuefinder__rename-modal__item-name' },
  Tt = /* @__PURE__ */ X({
    __name: 'ModalRename',
    setup(t) {
      const e = ee(),
        ...
        l = e.fs,
        i = W(l.path),
        r = E(e.modal.data.items[0]),
        a = E(r.value.basename),
        f = E(''),
        c = () => {
          a.value != r.value.basename &&
            e.adapter
              .rename({
                path: i.value.path,
                item: r.value.path,
  ...
  Os = { class: 'vuefinder__text-preview' },
  Ls = { class: 'vuefinder__text-preview__header' },
  Rs = ['title'],
  Vs = { class: 'vuefinder__text-preview__actions' },
  Ps = {
    key: 0,
        ...
        w('div', Os, [
          o('div', Ls, [
            o(
              ...
              y(s(_).modal.data.item.basename),
              9,
              Rs
            ),
            o('div', Vs, [
              a.value
                ...
                : M('', !0),
              s(_).features.includes(s(ne).EDIT)
                ? (u(),
                  w(
                    'button',
                    {
  ...
  Hs = { class: 'vuefinder__image-preview' },
  Ns = { class: 'vuefinder__image-preview__header' },
  Us = ['title'],
  Ks = { class: 'vuefinder__image-preview__actions' },
  Ws = { class: 'vuefinder__image-preview__image-container' },
  js = ['src'],
  Gs = /* @__PURE__ */ X({
    name: 'ImagePreview',
    __name: 'Image',
    ...
    setup(t, { emit: e }) {
      const n = e,
        ...
        a = E(''),
        f = E(!1),
        c = E(l.adapter.getPreviewUrl({ path: l.modal.data.item.path })),
        _ = E(c.value),
        v = Ke('cropperRef'),
        d = async () => {
          ...
          D &&
            D.toBlob(async (g) => {
              if (g) {
                ((a.value = ''), (f.value = !1));
                try {
                  const p = new File([g], l.modal.data.item.basename, { type: 'image/png' }),
                    k = l.modal.data.item.path.split('/'),
                    C = k.pop(),
                    $ = k.join('/');
                  ...
                  (O && O instanceof HTMLElement && Ln.resetStatus(O),
          ...
          w('div', Hs, [
            o('div', Ns, [
              o(
                ...
                y(s(l).modal.data.item.basename),
                9,
                Us
              ),
              o('div', Ks, [
                r.value
                  ...
                  : M('', !0),
                s(l).features.includes(s(ne).EDIT)
                  ? (u(),
                    w(
                      'button',
                      {
...
const wt = { render: Ys },
  Qs = { class: 'vuefinder__default-preview' },
  Xs = { class: 'vuefinder__default-preview__content' },
  Js = { class: 'vuefinder__default-preview__header' },
  Zs = ['title'],
  el = { class: 'vuefinder__default-preview__icon-container' },
  tl = ['title'],
  nl = /* @__PURE__ */ X({
    __name: 'Default',
    emits: ['success'],
            ...
            o('div', Xs, [
              o('div', Js, [
                o(
                  ...
                  y(s(n).modal.data.item.basename),
                  9,
                  Zs
                ),
              ]),
  ...
  ol = { class: 'vuefinder__video-preview' },
  sl = ['title'],
  ll = {
  ...
  al = /* @__PURE__ */ X({
    ...
    setup(t, { emit: e }) {
      const n = ee(),
        l = e,
        i = () => n.adapter.getPreviewUrl({ path: n.modal.data.item.path });
      return (
        ve(() => {
          l('success');
        }),
  ...
  rl = { class: 'vuefinder__audio-preview' },
  dl = ['title'],
  cl = {
  ...
  fl = { class: 'vuefinder__pdf-preview' },
  _l = ['title'],
  ml = ['data'],
  pl = ['src'],
  hl = /* @__PURE__ */ X({
...
const wl = {
  ...
  yl = ['disabled', 'title'],
  bl = ['disabled', 'title'],
  xl = { class: 'vuefinder__preview-modal__content' },
  kl = { key: 0 },
  ...
  Sl = { class: 'vuefinder__preview-modal__details' },
  Fl = { class: 'font-bold' },
  Dl = { class: 'font-bold pl-2' },
  El = {
    key: 0,
  ...
  It = /* @__PURE__ */ X({
    ...
    setup(t) {
      const e = ee(),
        { t: n } = e.i18n,
        l = E(!1),
        i = (g) => (e.modal.data.item.mime_type ?? '').startsWith(g),
        r = e.features.includes(ne.PREVIEW);
      r || (l.value = !0);
      const a = U(() => e.modal.data.item),
        f = W(e.fs.sortedFiles),
        c = U(() => f.value.filter((g) => g.type === 'file')),
        _ = U(() => c.value.findIndex((g) => g.path === a.value.path)),
        v = U(() => _.value > 0),
        d = U(() => _.value < c.value.length - 1),
        h = () => {
          ...
          g &&
            ...
            e.fs.select(g.path),
            (e.modal.data.item = g),
            (e.modal.data.storage = e.modal.data.storage));
        },
        b = () => {
        ...
        D = (g) => {
          ...
          (g.key === 'ArrowLeft' || g.key === 'ArrowRight') &&
            (g.preventDefault(), g.stopPropagation(), g.key === 'ArrowLeft' ? h() : b());
        };
      return (
        ve(() => {
        ...
        (g, p) => (
          ...
          V(Ee, null, {
            buttons: Q(() => [
              ...
              s(e).features.includes(s(ne).DOWNLOAD)
                ? (u(),
                  w(
                    'a',
                    {
...
const on = { render: Wl },
  ...
  Gl = { class: 'vuefinder__modal-tree__folder-content' },
  ql = {
    key: 1,
    class: 'vuefinder__modal-tree__folder-spacer',
  },
  Yl = { class: 'vuefinder__modal-tree__folder-text' },
  Ql = {
    key: 0,
  ...
  Jl = /* @__PURE__ */ X({
    ...
    setup(t, { emit: e }) {
      ...
      const f = U(() => {
        ...
        c = U(() => r.modelValue?.path === r.folder.path),
        _ = U(() => r.currentPath?.path === r.folder.path),
        v = U(() => r.modalTreeData[r.folder.path] || []),
        d = U(() => v.value.length > 0 || r.folder.type === 'dir'),
        h = () => {
                      ...
                      ($) => (
                        ...
                        V(
                          C,
                          {
                            ...
                            'onUpdate:modelValue':
                              k[0] || (k[0] = (O) => m.$emit('update:modelValue', O)),
                            onSelectAndClose: k[1] || (k[1] = (O) => m.$emit('selectAndClose', O)),
                            onToggleFolder:
                              k[2] || (k[2] = (O, z) => m.$emit('toggleFolder', O, z)),
                          },
                          null,
                          8,
                          [
  ...
  Zl = { class: 'vuefinder__modal-tree' },
  ei = { class: 'vuefinder__modal-tree__header' },
  ti = { class: 'vuefinder__modal-tree__title' },
  ni = {
    key: 0,
    class: 'vuefinder__modal-tree__section',
  },
  oi = { class: 'vuefinder__modal-tree__section-title' },
  si = { class: 'vuefinder__modal-tree__list' },
  li = ['onClick', 'onDblclick', 'onTouchend'],
  ii = { class: 'vuefinder__modal-tree__text' },
  ai = { class: 'vuefinder__modal-tree__text-storage' },
  ri = { class: 'vuefinder__modal-tree__section-title' },
  di = { class: 'vuefinder__modal-tree__list' },
  ci = { class: 'vuefinder__modal-tree__storage-item' },
  ui = { class: 'vuefinder__modal-tree__storage-content' },
  vi = ['onClick'],
  fi = ['onClick', 'onDblclick', 'onTouchend'],
  _i = { class: 'vuefinder__modal-tree__storage-text' },
  mi = {
    key: 0,
    class: 'vuefinder__modal-tree__subfolders',
  },
    ...
    setup(t, { emit: e }) {
      ...
      return (
        ...
        (L, G) => (
          u(),
          w('div', Zl, [
            o('div', ei, [o('div', ti, y(s(l)('Select Target Folder')), 1)]),
            o(
              'div',
              {
              ...
              [
                t.showPinnedFolders && s(r).get('pinnedFolders').length
                  ? (u(),
                    w('div', ni, [
  ...
  pi = { class: 'vuefinder__move-modal__content' },
  hi = { class: 'vuefinder__move-modal__description' },
  gi = { class: 'vuefinder__move-modal__files vf-scrollbar' },
  wi = {
    key: 0,
  ...
  bi = { class: 'vuefinder__move-modal__file-name' },
  xi = { class: 'vuefinder__move-modal__target-title' },
  ki = { class: 'vuefinder__move-modal__target-container' },
  $i = { class: 'vuefinder__move-modal__target-path' },
  Ci = { class: 'vuefinder__move-modal__target-storage' },
  Si = {
    key: 0,
    class: 'vuefinder__move-modal__Destination-folder',
  },
  Fi = { class: 'vuefinder__move-modal__target-badge' },
  Di = { class: 'vuefinder__move-modal__options' },
  Ei = { class: 'vuefinder__move-modal__checkbox-label' },
  Ai = { class: 'vuefinder__move-modal__checkbox-text' },
  Mi = { class: 'vuefinder__move-modal__selected-items' },
  Nn = /* @__PURE__ */ X({
    ...
    setup(t) {
      const e = ee(),
        { t: n } = e.i18n,
        l = t,
        i = E(e.modal.data.items.from),
        r = E(e.modal.data.items.to),
        a = E(''),
        f = E(l.copy || !1),
        c = U(() => (f.value ? 'copy' : 'move')),
        _ = E(!1),
        v = U(() => (f.value ? n('Copy files') : n('Move files'))),
        d = U(() =>
          f.value
            ? n('Are you sure you want to copy these files?')
            : n('Are you sure you want to move these files?')
        ),
      ...
      const b = (m) => {
        ...
        g = () => {
          ...
          if (m.endsWith('://')) return { storage: m.replace('://', ''), path: '' };
          const k = m.split('://');
          return {
            storage: k[0] || 'local',
            path: k[1] || '',
                ...
                o(
                  ...
                  [
                    I(
                      ...
                      {
                        modelValue: r.value,
                        'onUpdate:modelValue': [k[1] || (k[1] = (C) => (r.value = C)), b],
...
const Rt = { render: Ri },
  Vi = { class: 'vuefinder__search-modal__search-input' },
  Pi = ['value', 'placeholder', 'disabled'],
  Bi = {
    key: 0,
      ...
      return (
        ...
        (_, v) => (
          ...
          w('div', Vi, [
            ...
            t.isSearching
              ? (u(), w('div', Bi, [I(s(Rt), { class: 'vuefinder__search-modal__loading-icon' })]))
              : M('', !0),
          ])
        )
      );
...
function Ki(t, e, n) {
  ...
  const l = Pt(t),
    ...
    r = Wn(i);
  let a =
    i === 'x' ? (l === (n ? 'end' : 'start') ? 'right' : 'left') : l === 'start' ? 'bottom' : 'top';
...
function kt(t) {
  const { x: e, y: n, width: l, height: i } = t;
  return {
    width: l,
    height: i,
    top: n,
...
const Ji = async (t, e, n) => {
  const { placement: l = 'bottom', strategy: i = 'absolute', middleware: r = [], platform: a } = n,
    f = r.filter(Boolean),
    c = await (a.isRTL == null ? void 0 : a.isRTL(e));
...
async function Gn(t, e) {
  var n;
  e === void 0 && (e = {});
  const { x: l, y: i, platform: r, rects: a, elements: f, strategy: c } = t,
    {
    ...
    D = Xi(b),
    p = f[h ? (d === 'floating' ? 'reference' : 'floating') : d],
    m = kt(
      await r.getClippingRect({
        element:
          (n = await (r.isElement == null ? void 0 : r.isElement(p))) == null || n
            ? p
            : p.contextElement ||
              (await (r.getDocumentElement == null ? void 0 : r.getDocumentElement(f.floating))),
        boundary: _,
        rootBoundary: v,
        strategy: c,
      })
    ...
    k =
      d === 'floating'
        ...
        : a.reference,
    C = await (r.getOffsetParent == null ? void 0 : r.getOffsetParent(f.floating)),
    $ = (await (r.isElement == null ? void 0 : r.isElement(C)))
      ? (await (r.getScale == null ? void 0 : r.getScale(C))) || {
          x: 1,
          y: 1,
        }
      : {
      ...
      t === void 0 && (t = {}),
      {
        ...
        async fn(e) {
          ...
          const m = Xe(i),
            k = je(f),
            C = Xe(f) === f,
            $ = await (c.isRTL == null ? void 0 : c.isRTL(_.floating)),
            O = h || (C || !g ? [xt(f)] : Wi(f)),
            z = D !== 'none';
          !h && z && O.push(...Yi(f, g, D, $));
          const L = [f, ...O],
          ...
          if (
            ...
            var re, pe;
            const le = (((re = r.flip) == null ? void 0 : re.index) || 0) + 1,
              _e = L[le];
            if (
              _e &&
              (!(d === 'alignment' ? k !== je(_e) : !1) || // We leave the current main axis only if every placement on that axis
            ...
            let Z =
              (pe = q
                ...
                .sort((S, x) => S.overflows[1] - x.overflows[1])[0]) == null
                ? void 0
                : pe.placement;
            if (!Z)
              switch (b) {
                    ...
                    (J = q
                      .filter((x) => {
                        if (z) {
                          ...
                          return (
                            F === k || // Create a bias to the `y` side axis due to horizontal
                      ...
                      .map((x) => [
                        x.placement,
                        x.overflows.filter((F) => F > 0).reduce((F, A) => F + A, 0),
                      ])
                      .sort((x, F) => x[1] - F[1])[0]) == null
                      ? void 0
                      : J[0];
                  S && (Z = S);
                  break;
...
async function ta(t, e) {
  const { placement: n, platform: l, elements: i } = t,
    r = await (l.isRTL == null ? void 0 : l.isRTL(i.floating)),
    a = Xe(n),
    f = Pt(n),
    c = je(n) === 'y',
    _ = ea.has(a) ? -1 : 1,
  ...
  return (
    f && typeof D == 'number' && (b = f === 'end' ? D * -1 : D),
    c
      ? {
          x: b * v,
          y: h * _,
...
const na = function (t) {
    return (
      t === void 0 && (t = 0),
      {
        ...
        async fn(e) {
          var n, l;
          const { x: i, y: r, placement: a, middlewareData: f } = e,
            c = await ta(e, t);
          return a === ((n = f.offset) == null ? void 0 : n.placement) &&
            (l = f.arrow) != null &&
            l.alignmentOffset
            ? {}
            : {
...
function Pe(t) {
  var e;
  return (e = (qn(t) ? t.ownerDocument : t.document) || window.document) == null
    ? void 0
    : e.documentElement;
}
function qn(t) {
...
function _t(t) {
  const { overflow: e, overflowX: n, overflowY: l, display: i } = Oe(t);
...
const ra = ['transform', 'translate', 'scale', 'rotate', 'perspective'],
  da = ['transform', 'translate', 'scale', 'rotate', 'perspective', 'filter'],
  ca = ['paint', 'layout', 'strict', 'content'];
function rn(t) {
  const e = dn(),
    n = Ie(t) ? Oe(t) : t;
  return (
    ra.some((l) => (n[l] ? n[l] !== 'none' : !1)) ||
    (n.containerType ? n.containerType !== 'normal' : !1) ||
    (!e && (n.backdropFilter ? n.backdropFilter !== 'none' : !1)) ||
    (!e && (n.filter ? n.filter !== 'none' : !1)) ||
    da.some((l) => (n.willChange || '').includes(l)) ||
    ca.some((l) => (n.contain || '').includes(l))
  );
}
function ua(t) {
  let e = Ge(t);
...
function ct(t, e, n) {
  ...
  const i = Yn(t),
    r = i === ((l = t.ownerDocument) == null ? void 0 : l.body),
    a = Fe(i);
  if (r) {
    const f = Qt(a);
...
function et(t) {
  ...
  const n = e.getBoundingClientRect(),
    { width: l, height: i, $: r } = Qn(e);
  let a = (r ? bt(n.width) : n.width) / l,
    f = (r ? bt(n.height) : n.height) / i;
  return (
  ...
  if (r) {
    ...
    for (; g && l && b !== D; ) {
      const p = et(g),
        ...
        k = Oe(g),
        C = m.left + (g.clientLeft + parseFloat(k.paddingLeft)) * p.x,
        $ = m.top + (g.clientTop + parseFloat(k.paddingTop)) * p.y;
      ((c *= p.x),
        (_ *= p.y),
        (v *= p.x),
        (d *= p.y),
...
function ma(t) {
  let { elements: e, rect: n, offsetParent: l, strategy: i } = t;
  const r = i === 'fixed',
    a = Pe(l),
    f = e ? zt(e.floating) : !1;
  if (l === a || (f && r)) return n;
...
function ha(t) {
  const e = Pe(t),
    ...
    i = Qe(e.scrollWidth, e.clientWidth, l.scrollWidth, l.clientWidth),
    r = Qe(e.scrollHeight, e.clientHeight, l.scrollHeight, l.clientHeight);
  let a = -n.scrollLeft + Nt(t);
  const f = -n.scrollTop;
  return (
    Oe(l).direction === 'rtl' && (a += Qe(e.clientWidth, l.clientWidth) - i),
    {
      width: i,
      height: r,
      x: a,
...
function ga(t, e) {
  ...
  if (_ <= 0) {
    const v = l.ownerDocument,
      ...
      b =
        (v.compatMode === 'CSS1Compat' && parseFloat(h.marginLeft) + parseFloat(h.marginRight)) ||
        0,
      D = Math.abs(l.clientWidth - d.clientWidth - b);
    D <= $n && (r -= D);
  } else _ <= $n && (r += _);
  return {
    width: r,
...
function ya(t, e) {
  const n = Je(t, !0, e === 'fixed'),
    l = n.top + t.clientTop,
    i = n.left + t.clientLeft,
    r = Ve(t) ? et(t) : Le(1),
    a = t.clientWidth * r.x,
    f = t.clientHeight * r.y,
    c = i * r.x,
    _ = l * r.y;
  return {
    width: a,
...
function Cn(t, e, n) {
  let l;
  if (e === 'viewport') l = ga(t, n);
  else if (e === 'document') l = ha(Pe(t));
  else if (Ie(e)) l = ya(e, n);
  else {
    const i = Xn(t);
    l = {
      x: e.x - i.x,
...
function ba(t, e) {
  const n = e.get(t);
  if (n) return n;
  let l = ct(t, [], !1).filter((f) => Ie(f) && ot(f) !== 'body'),
    i = null;
  ...
  for (; Ie(a) && !nt(a); ) {
    const f = Oe(a),
      c = rn(a);
    (!c && f.position === 'fixed' && (i = null),
      (
        r
          ? !c && !i
          : (!c && f.position === 'static' && !!i && wa.has(i.position)) ||
            (_t(a) && !c && Zn(t, a))
      )
        ? (l = l.filter((v) => v !== a))
        : (i = f),
...
function xa(t) {
  let { element: e, boundary: n, rootBoundary: l, strategy: i } = t;
  const a = [...(n === 'clippingAncestors' ? (zt(e) ? [] : ba(e, this._c)) : [].concat(n)), l],
    f = a[0],
    c = a.reduce(
      (_, v) => {
        const d = Cn(e, v, i);
        return (
          (_.top = Qe(d.top, _.top)),
          (_.right = yt(d.right, _.right)),
          (_.bottom = yt(d.bottom, _.bottom)),
          (_.left = Qe(d.left, _.left)),
          _
        );
      },
      Cn(e, f, i)
...
function $a(t, e, n) {
  ...
  r && !l && i && _();
  const v = i && !l && !r ? Jn(i, f) : Le(0),
    d = a.left + f.scrollLeft - c.x - v.x,
    h = a.top + f.scrollTop - c.y - v.y;
  return {
    x: d,
    y: h,
    width: a.width,
...
function Sn(t, e) {
  if (!Ve(t) || Oe(t).position === 'fixed') return null;
  if (e) return e(t);
...
function Da(t, e) {
  ...
  function a(f, c) {
    ...
    const _ = t.getBoundingClientRect(),
      { left: v, top: d, width: h, height: b } = _;
    if ((f || e(), !h || !b)) return;
    const D = gt(d),
      g = gt(i.clientWidth - (v + h)),
      p = gt(i.clientHeight - (d + b)),
      m = gt(v),
      C = {
...
const oo = { render: Aa },
  Ma = ['disabled', 'title'],
  Ta = ['data-theme'],
  Ia = { class: 'vuefinder__search-modal__dropdown-content' },
  Oa = { class: 'vuefinder__search-modal__dropdown-section' },
  La = { class: 'vuefinder__search-modal__dropdown-title' },
  Ra = { class: 'vuefinder__search-modal__dropdown-options' },
  Va = {
    key: 0,
    ...
    setup(t, { expose: e, emit: n }) {
      ...
      const v = (g) => {
        ...
        d = async () => {
          l.disabled ||
            (l.visible
              ? (i('update:visible', !1), _ && (_(), (_ = null)))
              : (i('update:visible', !0), await Re(), await h()));
        },
        h = async () => {
        ...
        b = (g) => {
          ...
          const p = ['size-all', 'size-small', 'size-medium', 'size-large'],
            m = p.findIndex((k) => k === l.selectedOption);
          if (g.key === 'ArrowDown') {
            g.preventDefault();
            ...
            g.key === 'Enter'
              ? (g.preventDefault(),
                l.selectedOption?.startsWith('size-') &&
                  i('update:sizeFilter', l.selectedOption.split('-')[1]))
              : g.key === 'Escape' &&
                (g.preventDefault(), i('update:visible', !1), _ && (_(), (_ = null)));
        },
        D = () => {
          _ && (_(), (_ = null));
        };
              ...
              o(
                'button',
                {
                  ...
                  class: j([
                    ...
                    { 'vuefinder__search-modal__dropdown-btn--active': t.visible },
                  ]),
                  disabled: t.disabled,
...
function so(t, e = 40) {
  ...
  const c = a.split(/\.(?=[^\.]+$)/),
    _ = c[0] ?? '',
    v = c[1] ?? '',
    d = _.length > 10 ? `${_.slice(0, 6)}...${_.slice(-5)}` : _,
    h = v ? `${d}.${v}` : d;
  return (
    (f = `${l}${r.join('/')}${r.length ? '/' : ''}${h}`),
    f.length > e && (f = `${l}.../${h}`),
    f
  );
}
...
const io = { render: ja },
  Ga = ['title'],
  qa = { class: 'vuefinder__search-modal__result-icon' },
  Ya = { class: 'vuefinder__search-modal__result-content' },
  Qa = { class: 'vuefinder__search-modal__result-name' },
  Xa = {
    key: 0,
  ...
  Ja = ['title'],
  Za = ['title'],
  er = ['data-item-dropdown', 'data-theme'],
  tr = { class: 'vuefinder__search-modal__item-dropdown-content' },
  nr = /* @__PURE__ */ X({
    ...
    setup(t, { emit: e }) {
      ...
      const c = (m) => n.expandedPaths.has(m),
        _ = (m) => (m.type === 'dir' || !m.file_size ? '' : en(m.file_size)),
        v = (m, k) => {
          (k.stopPropagation(), l('toggleItemDropdown', m, k));
        },
        d = async (m, k) => {
          const C = document.querySelector(`[data-item-dropdown="${m}"]`);
          if (!(!C || !k) && (await Re(), !(!C || !k))) {
        ...
        p = (m) => {
          ...
          const k = ['copy-path', 'open-folder', 'preview'],
            C = n.selectedItemDropdownOption,
            $ = k.findIndex((O) => C?.includes(O));
          if (m.key === 'ArrowDown') {
            m.preventDefault();
            const O = ($ + 1) % k.length;
            l('update:selectedItemDropdownOption', `${k[O] || ''}-${n.activeDropdown}`);
            ...
            m.key === 'Enter'
              ? (m.preventDefault(),
                C &&
                  (C.includes('copy-path')
                    ? b(n.item)
                    : C.includes('open-folder')
                      ? D(n.item)
                      : C.includes('preview') && g(n.item)))
              : m.key === 'Escape' &&
                (m.preventDefault(), l('update:selectedItemDropdownOption', null));
        };
      return (m, k) => (
        u(),
        w(
          'div',
          {
            class: j([
              ...
              { 'vuefinder__search-modal__result-item--selected': t.index === t.selectedIndex },
            ]),
          ...
          [
            ...
            o('div', Ya, [
              ...
              o(
                ...
                y(c(t.item.path) ? t.item.path : s(so)(t.item.path)),
                9,
                Ja
              ),
            ]),
                            ...
                            class: j([
                              'vuefinder__search-modal__item-dropdown-option',
                              {
                                'vuefinder__search-modal__item-dropdown-option--selected':
                                  t.selectedItemDropdownOption === `copy-path-${t.item.path}`,
                              },
                            ]),
                            onClick:
                              k[2] ||
                        ...
                        o(
                          'div',
                          {
                            class: j([
                              ...
                              {
                                'vuefinder__search-modal__item-dropdown-option--selected':
                                  t.selectedItemDropdownOption === `open-folder-${t.item.path}`,
                              },
                            ]),
                            onClick:
                              k[4] ||
                        ...
                        o(
                          'div',
                          {
                            class: j([
                              ...
                              {
                                'vuefinder__search-modal__item-dropdown-option--selected':
                                  t.selectedItemDropdownOption === `preview-${t.item.path}`,
                              },
                            ]),
                            onClick:
                              k[6] ||
  ...
  sr = { class: 'vuefinder__search-modal__loading-icon' },
  lr = {
    key: 1,
  ...
  ar = { class: 'vuefinder__search-modal__results-header' },
  Ye = 60,
  Fn = 5,
  rr = /* @__PURE__ */ X({
    ...
    setup(t, { expose: e, emit: n }) {
      const l = t,
        ...
        r = ee(),
        { t: a } = r.i18n,
        f = Ke('scrollableContainer'),
        c = U(() => l.searchResults.length > 0),
        _ = U(() => l.searchResults.length),
        v = E(0),
        d = E(600),
        h = U(() => l.searchResults.length * Ye),
        b = U(() => {
          const C = Math.max(0, Math.floor(v.value / Ye) - Fn),
            $ = Math.min(l.searchResults.length, Math.ceil((v.value + d.value) / Ye) + Fn);
          return { start: C, end: $ };
        }),
        D = U(() => {
        ...
        m = () => {
          if (l.selectedIndex >= 0 && f.value) {
            const C = l.selectedIndex * Ye,
              $ = C + Ye,
              O = f.value.scrollTop,
              z = f.value.clientHeight,
              L = O + z;
            let G = O;
            (C < O ? (G = C) : $ > L && (G = $ - z),
              G !== O &&
          ...
          w(
            'div',
            {
              class: j([
                ...
                { 'vuefinder__search-modal__results--enter': t.resultsEnter },
              ]),
            },
            [
              t.isSearching
                ? (u(),
                  w('div', or, [
                    o('div', sr, [I(s(Rt), { class: 'vuefinder__search-modal__loading-icon' })]),
                    o('span', null, y(s(a)('Searching...')), 1),
                  ]))
                : c.value
                  ? (u(),
                    w('div', ir, [
                      o('div', ar, [o('span', null, y(s(a)('Found %s results', _.value)), 1)]),
                      o(
                        'div',
                        {
                                        ...
                                        I(
                                          nr,
                                          {
                                            ...
                                            'selected-item-dropdown-option':
                                              t.selectedItemDropdownOption,
                                            onSelect:
                                              $[0] || ($[0] = (z) => i('selectResultItem', z)),
                                            onSelectWithDropdown:
                                              $[1] ||
                                              ($[1] = (z) => i('selectResultItemWithDropdown', z)),
                                            onTogglePathExpansion:
                                              $[2] || ($[2] = (z) => i('togglePathExpansion', z)),
                                            onToggleItemDropdown:
                                              $[3] ||
                                              ($[3] = (z, L) => i('toggleItemDropdown', z, L)),
                                            'onUpdate:selectedItemDropdownOption':
                                              $[4] ||
                                              ($[4] = (z) =>
                                                i('update:selectedItemDropdownOption', z)),
                                            onCopyPath: $[5] || ($[5] = (z) => i('copyPath', z)),
                                            onOpenContainingFolder:
                                              $[6] || ($[6] = (z) => i('openContainingFolder', z)),
                  ...
                  : (u(), w('div', lr, [o('span', null, y(s(a)('No results found')), 1)])),
            ],
            2
          )
        )
  ...
  dr = { class: 'vuefinder__search-modal' },
  cr = { class: 'vuefinder__search-modal__content' },
  ur = { class: 'vuefinder__search-modal__search-bar' },
  vr = { class: 'vuefinder__search-modal__search-location' },
  fr = ['title'],
  _r = ['disabled'],
  mr = {
    key: 0,
  ...
  pr = { class: 'vuefinder__search-modal__folder-selector-content' },
  hr = {
    key: 1,
    class: 'vuefinder__search-modal__instructions',
  },
  gr = { class: 'vuefinder__search-modal__instructions-tips' },
  wr = { class: 'vuefinder__search-modal__tip' },
  yr = { class: 'vuefinder__search-modal__tip' },
  un = /* @__PURE__ */ X({
    name: 'ModalSearch',
    __name: 'ModalSearch',
    setup(t) {
      const e = ee(),
        ...
        z = (x, F) => {
          (F && typeof F.stopPropagation == 'function' && F.stopPropagation(),
            C.value === x ? (C.value = null) : (C.value = x));
        },
        L = () => {
          C.value = null;
        },
      ...
      (ue(f, async (x) => {
        x.trim()
          ? (await J(x.trim()), (v.value = 0))
          : ((c.value = []), (_.value = !1), (v.value = -1));
      }),
        ue(D, async (x) => {
          ((p.value = `size-${x}`),
      ...
      const J = async (x) => {
        if (x) {
          _.value = !0;
          try {
            const F = b.value?.path || $?.value?.path,
              A = await e.adapter.search({
                path: F,
                filter: x,
                deep: g.value,
                size: D.value,
      ...
      const le = () => {
          h.value
            ? ((h.value = !1), f.value.trim() && (J(f.value.trim()), (v.value = 0)))
            : ((d.value = !1), (h.value = !0));
        },
        _e = (x) => {
          x && (b.value = x);
      ...
      const S = (x) => {
        ...
        if (
          (d.value &&
            (F.closest('.vuefinder__search-modal__dropdown') ||
              ((d.value = !1),
              Re(() => {
                i.value && i.value.focus();
              }))),
          ...
          const A = F.closest('.vuefinder__search-modal__item-dropdown'),
            H = F.closest('.vuefinder__search-modal__result-item');
          !A && !H && L();
        }
      };
      return (x, F) => (
                        ...
                        o(
                          'button',
                          {
                            ...
                            class: j([
                              ...
                              { 'vuefinder__search-modal__location-btn--open': h.value },
                            ]),
                          },
                          [
                      ...
                      w('div', mr, [
                        o('div', pr, [
                          I(
                            ...
                            {
                              modelValue: b.value,
                              'onUpdate:modelValue': [F[8] || (F[8] = (A) => (b.value = A)), _e],
                              'show-pinned-folders': !0,
                              'current-path': s($),
                              onSelectAndClose: Z,
                            },
  ...
  const e = t.fs,
    ...
    i = (r) => {
      if ((r.code === Se.ESCAPE && (t.modal.close(), t.root.focus()), !t.modal.visible)) {
        if (
          (r.code === Se.F2 &&
            t.features.includes(ne.RENAME) &&
            l.value.length === 1 &&
            t.modal.open(Tt, { items: l.value }),
          r.code === Se.F5 && t.adapter.open(e.path.get().path),
          r.code === Se.DELETE && l.value.length === 0 && t.modal.open(Mt, { items: l.value }),
          r.ctrlKey && r.code === Se.BACKSLASH && t.modal.open(Bn),
          r.ctrlKey &&
            r.code === Se.KEY_F &&
            t.features.includes(ne.SEARCH) &&
            (t.modal.open(un), r.preventDefault()),
          r.ctrlKey && r.code === Se.KEY_E && (n.toggle('showTreeView'), r.preventDefault()),
          r.ctrlKey && r.code === Se.ENTER && (n.toggle('fullScreen'), t.root.focus()),
          r.ctrlKey &&
            r.code === Se.KEY_A &&
            (e.selectAll(t.selectionMode || 'multiple', t), r.preventDefault()),
          r.code === Se.SPACE &&
            l.value.length === 1 &&
            l.value[0]?.type !== 'dir' &&
            t.modal.open(It, {
          ...
          (e.setClipboard('copy', new Set(l.value.map((a) => a.path))),
            t.emitter.emit('vf-toast-push', {
              label:
                l.value.length === 1
                  ? t.i18n.t('Item copied to clipboard')
                  : t.i18n.t('%s items copied to clipboard', l.value.length),
            }),
            r.preventDefault());
        }
        if (r.metaKey && r.code === Se.KEY_X) {
          ...
          (e.setClipboard('cut', new Set(l.value.map((a) => a.path))),
            t.emitter.emit('vf-toast-push', {
              label:
                l.value.length === 1
                  ? t.i18n.t('Item cut to clipboard')
                  : t.i18n.t('%s items cut to clipboard', l.value.length),
            }),
            r.preventDefault());
        }
...
function xr() {
  ...
  return {
    ...
    handleDragEnter: (f) => {
      ...
      c &&
        Array.from(c).some((v) => v.kind === 'file') &&
        ((t.value = !0), (f.isExternalDrag = !0));
    },
    handleDragOver: (f) => {
      t.value &&
        f.dataTransfer &&
        ((f.dataTransfer.dropEffect = 'copy'), f.preventDefault(), f.stopPropagation());
    },
    handleDragLeave: (f) => {
      f.preventDefault();
...
const ao = { render: $r },
  Cr = { class: 'vuefinder__new-folder-modal__content' },
  Sr = { class: 'vuefinder__new-folder-modal__form' },
  Fr = { class: 'vuefinder__new-folder-modal__description' },
  Dr = ['placeholder'],
  fn = /* @__PURE__ */ X({
...
const ro = { render: Ar },
  Mr = { class: 'vuefinder__new-file-modal__content' },
  Tr = { class: 'vuefinder__new-file-modal__form' },
  Ir = { class: 'vuefinder__new-file-modal__description' },
  Or = ['placeholder'],
  co = /* @__PURE__ */ X({
    __name: 'ModalNewFile',
    setup(t) {
            ...
            : (u(),
              w(
                'div',
                {
                  ...
                  class: j([
                    'vuefinder__message',
                    t.error ? 'vuefinder__message--error' : 'vuefinder__message--success',
                  ]),
                },
                [
                  De(_.$slots, 'default'),
...
function Vr(t) {
  ...
  const k = (S) => {
    ...
    $ = (S) => {
      (S.preventDefault(),
        S.stopPropagation(),
        (!S.relatedTarget || S.relatedTarget === document.body) && (g.value = !1));
    },
    O = (S) => {
    ...
    z = (S) => h.value.findIndex((x) => x.id === S),
    L = (S, x) => m.addFile({ name: x || S.name, type: S.type, data: S, source: 'Local' }),
    G = (S) =>
      S.status === ye.DONE
        ? 'text-green-600'
        : S.status === ye.ERROR || S.status === ye.CANCELED
          ? 'text-red-600'
          : '',
    R = (S) =>
      S.status === ye.DONE ? '✓' : S.status === ye.ERROR || S.status === ye.CANCELED ? '!' : '...',
    q = () => v.value?.click(),
    re = () => e.modal.close(),
    pe = (S) => {
  ...
  return (
    ve(() => {
      m = new Ao({
        ...
        onBeforeFileAdded: (A, H) => {
          if (H[A.id] != null) {
            const he = z(A.id);
            (h.value[he]?.status === ye.PENDING &&
              (b.value = m.i18n('noDuplicates', { fileName: A.name })),
              (h.value = h.value.filter((ce) => ce.id !== A.id)));
          }
          return (
            h.value.push({
              id: A.id,
      ...
      (m.on('restriction-failed', (A, H) => {
        ...
        m.on('upload-progress', (A, H) => {
          const Y = H.bytesTotal ?? 1,
            he = Math.floor((H.bytesUploaded / Y) * 100),
            ce = z(A.id);
        ...
        m.on('upload-error', (A, H) => {
          ...
          Y &&
            ((Y.percent = null),
            (Y.status = ye.ERROR),
            (Y.statusName = H?.isNetworkError
              ? n('Network Error, Unable establish connection to the server or interrupted.')
              : H?.message || n('Unknown Error')));
        }),
        m.on('error', (A) => {
        ...
        v.value?.addEventListener('click', () => c.value?.click()),
        d.value?.addEventListener('click', () => _.value?.click()));
      const x = { capture: !0 };
      (document.addEventListener('dragover', k, x),
        document.addEventListener('dragenter', C, x),
        document.addEventListener('dragleave', $, x),
        document.addEventListener('drop', O, x));
      const F = (A) => {
        const H = A.target,
          Y = H.files;
        if (Y) {
    ...
    xe(() => {
      const S = { capture: !0 };
      (document.removeEventListener('dragover', k, S),
        document.removeEventListener('dragenter', C, S),
        document.removeEventListener('dragleave', $, S),
        document.removeEventListener('drop', O, S));
    }),
    {
      container: f,
      internalFileInput: c,
...
const uo = { render: Br },
  zr = { class: 'vuefinder__upload-modal__content relative' },
  Hr = { class: 'vuefinder__upload-modal__target-section' },
  Nr = { class: 'vuefinder__upload-modal__target-label' },
  Ur = { class: 'vuefinder__upload-modal__target-container' },
  Kr = { class: 'vuefinder__upload-modal__target-path' },
  Wr = { class: 'vuefinder__upload-modal__target-storage' },
  jr = {
    key: 0,
    class: 'vuefinder__upload-modal__target-folder',
  },
  Gr = { class: 'vuefinder__upload-modal__target-badge' },
  qr = { class: 'vuefinder__upload-modal__drag-hint' },
  Yr = { class: 'vuefinder__upload-modal__file-list vf-scrollbar' },
  Qr = ['textContent'],
  Xr = { class: 'vuefinder__upload-modal__file-info' },
  Jr = { class: 'vuefinder__upload-modal__file-name hidden md:block' },
  Zr = { class: 'vuefinder__upload-modal__file-name md:hidden' },
  ed = {
    key: 0,
    class: 'ml-auto',
  },
  td = ['title', 'disabled', 'onClick'],
  nd = {
    key: 0,
    class: 'py-2',
  },
    ...
    setup(t) {
      const e = ee(),
        ...
        f = () => {
          ...
          if (S.endsWith('://')) return { storage: S.replace('://', ''), path: '' };
          const x = S.split('://');
          return {
            storage: x[0] || 'local',
            path: x[1] || '',
      ...
      const J = E(!1),
        ...
        Z = (S) => {
          if (!J.value) return;
          const x = S.target,
            F = le.value?.contains(x) ?? !1,
            A = _e.value?.contains(x) ?? !1;
          !F && !A && (J.value = !1);
        };
      return (
        ve(() => document.addEventListener('click', Z)),
        xe(() => document.removeEventListener('click', Z)),
        (S, x) => (
          u(),
          V(
            Ee,
                        ...
                        w('div', sd, [
                          ...
                          o(
                            'div',
                            {
                              ...
                              onClick:
                                x[7] || (x[7] = (F) => (s(p) ? null : (s(L)(!1), (J.value = !1)))),
                            },
                            y(s(n)('Clear all')),
                            3
                          ),
                          o(
                            'div',
                            {
                              class: j(['vuefinder__upload-actions__item', s(p) ? 'disabled' : '']),
                              onClick:
                                x[8] || (x[8] = (F) => (s(p) ? null : (s(L)(!0), (J.value = !1)))),
                            },
                            y(s(n)('Clear only successful')),
                            3
                          ),
                        ]))
                      : M('', !0),
                        ...
                        w('div', ad, [
                          ...
                          o(
                            'div',
                            {
                              ...
                              onClick:
                                x[14] ||
                                (x[14] = (F) => (s(p) ? null : (s(L)(!1), (J.value = !1)))),
                            },
                            y(s(n)('Clear all')),
                            3
                          ),
                          o(
                            'div',
                            {
                              ...
                              onClick:
                                x[15] ||
                                (x[15] = (F) => (s(p) ? null : (s(L)(!0), (J.value = !1)))),
                            },
                            y(s(n)('Clear only successful')),
                            3
                          ),
                ...
                o('div', null, [
                  ...
                  o('div', zr, [
                      ...
                      o(
                        ...
                        [
                          I(
                            ...
                            {
                              modelValue: r.value,
                              'onUpdate:modelValue': [x[1] || (x[1] = (F) => (r.value = F)), c],
                              'show-pinned-folders': !0,
                              onSelectAndClose: _,
                            },
                            null,
                    ...
                    o(
                      'div',
                      qr,
                      y(s(n)('You can drag & drop files anywhere while this modal is open.')),
                      1
                    ),
                    o(
                      'div',
                    ...
                    o('div', Yr, [
                              ...
                              [
                                ...
                                o('div', Xr, [
                                  ...
                                  o(
                                    ...
                                    [
                                      oe(y(F.statusName) + ' ', 1),
                                      F.status === s(k).QUEUE_ENTRY_STATUS.UPLOADING
                                        ? (u(), w('b', ed, y(F.percent), 1))
                                        : M('', !0),
                                    ],
                                    2
                                  ),
                      ...
                      s(D).length
                        ? M('', !0)
                        : (u(), w('div', nd, y(s(n)('No files selected!')), 1)),
                    ]),
                    s(g).length
                      ? (u(),
                        V(
...
const vo = { render: dd },
  cd = { class: 'vuefinder__unarchive-modal__content' },
  ud = { class: 'vuefinder__unarchive-modal__items' },
  vd = {
    key: 0,
  ...
  _d = { class: 'vuefinder__unarchive-modal__item-name' },
  md = { class: 'vuefinder__unarchive-modal__info' },
  mn = /* @__PURE__ */ X({
              ...
              o('div', cd, [
                o('div', ud, [
                  ...
                  o(
                    'p',
                    md,
                    y(s(i)('The archive will be unarchived at')) + ' (' + y(s(l).path) + ')',
                    1
                  ),
                  a.value.length
                    ? (u(),
...
const fo = { render: hd },
  gd = { class: 'vuefinder__archive-modal__content' },
  wd = { class: 'vuefinder__archive-modal__form' },
  yd = { class: 'vuefinder__archive-modal__files vf-scrollbar' },
  bd = {
    key: 0,
  ...
  kd = { class: 'vuefinder__archive-modal__file-name' },
  $d = ['placeholder'],
  pn = /* @__PURE__ */ X({
    __name: 'ModalArchive',
    setup(t) {
      const e = ee(),
        ...
        r = E(''),
        a = E(''),
        f = E(e.modal.data.items),
        c = () => {
          f.value.length &&
            e.adapter
              .archive({
                path: i.value.path,
                items: f.value.map(({ path: _, type: v }) => ({ path: _, type: v })),
                name: r.value,
              })
              .then((_) => {
    ...
    w(
      ...
      [
        t.$slots.default
          ...
          : (u(), w('span', Fd, y(l.t('Saved.')), 1)),
      ],
      2
    )
  );
}
const lt = /* @__PURE__ */ Sd(Cd, [['render', Dd]]),
  Ed = [
  ...
  Ad = { class: 'vuefinder__about-modal__content' },
  Md = { class: 'vuefinder__about-modal__main' },
  Td = { class: 'vuefinder__about-modal__description' },
  Id = { class: 'vuefinder__about-modal__settings' },
  Od = { class: 'vuefinder__about-modal__settings__fieldset' },
  Ld = { class: 'vuefinder__about-modal__settings__section-title' },
  Rd = { class: 'vuefinder__about-modal__setting' },
  Vd = { class: 'vuefinder__about-modal__setting-label' },
  Pd = {
    for: 'metric_unit',
    class: 'vuefinder__about-modal__label',
  },
  Bd = { class: 'vuefinder__about-modal__setting-input justify-end' },
  zd = ['checked'],
  Hd = { class: 'vuefinder__about-modal__setting' },
  Nd = { class: 'vuefinder__about-modal__setting-label' },
  Ud = {
    for: 'large_icons',
    class: 'vuefinder__about-modal__label',
  },
  Kd = { class: 'vuefinder__about-modal__setting-input justify-end' },
  Wd = ['checked'],
  jd = { class: 'vuefinder__about-modal__setting' },
  Gd = { class: 'vuefinder__about-modal__setting-label' },
  qd = {
    for: 'persist_path',
    class: 'vuefinder__about-modal__label',
  },
  Yd = { class: 'vuefinder__about-modal__setting-input justify-end' },
  Qd = ['checked'],
  Xd = { class: 'vuefinder__about-modal__settings__section-title' },
  Jd = { class: 'vuefinder__about-modal__setting' },
  Zd = { class: 'vuefinder__about-modal__setting-input justify-end' },
  ec = ['value'],
  tc = ['label'],
  nc = ['value'],
  oc = {
  ...
  lc = { class: 'vuefinder__about-modal__setting-input justify-end' },
  ic = ['label'],
  ac = ['value'],
  rc = { class: 'vuefinder__about-modal__tab-content' },
  dc = { class: 'vuefinder__about-modal__settings__section-title' },
  cc = { class: 'vuefinder__about-modal__description' },
  uc = /* @__PURE__ */ X({
    __name: 'ModalSettings',
    setup(t) {
      const e = ee(),
        n = e.config,
        { clearStore: l } = e.storage,
        { t: i } = e.i18n,
        r = W(n.state),
        a = U(() => r.value.theme || 'default'),
        f = async () => {
          (n.reset(), l(), location.reload());
        },
        c = (g) => {
          (g !== 'default' ? n.set('theme', g) : n.set('theme', 'default'),
            e.emitter.emit('vf-theme-saved'));
        },
        _ = () => {
          (n.toggle('metricUnits'),
            (e.filesize = n.get('metricUnits') ? Vn : en),
              ...
              o('div', Md, [
                ...
                o('div', Id, [
                  o('fieldset', Od, [
                    ...
                    o('div', Rd, [
                      o('div', Vd, [o('label', Pd, y(s(i)('Use Metric Units')), 1)]),
                      o('div', Bd, [
                        o(
                          'input',
                          {
                    ...
                    o('div', Hd, [
                      o('div', Nd, [o('label', Ud, y(s(i)('Compact list view')), 1)]),
                      o('div', Kd, [
                        o(
                          'input',
                          {
                    ...
                    o('div', jd, [
                      o('div', Gd, [o('label', qd, y(s(i)('Persist path on reload')), 1)]),
                      o('div', Yd, [
                        o(
                          'input',
                          {
                    ...
                    s(e).features.includes(s(ne).LANGUAGE) && Object.keys(s(D)).length > 1
                      ? (u(), w('div', oc, y(s(i)('Language')), 1))
                      : M('', !0),
                    s(e).features.includes(s(ne).LANGUAGE) && Object.keys(s(D)).length > 1
                      ? (u(),
                        w('div', sc, [
  ...
  vc = { class: 'vuefinder__about-modal__content' },
  fc = { class: 'vuefinder__about-modal__main' },
  _c = { class: 'vuefinder__about-modal__shortcuts' },
  mc = { class: 'vuefinder__about-modal__shortcut' },
  pc = { class: 'vuefinder__about-modal__shortcut' },
  hc = { class: 'vuefinder__about-modal__shortcut' },
  gc = { class: 'vuefinder__about-modal__shortcut' },
  wc = { class: 'vuefinder__about-modal__shortcut' },
  yc = { class: 'vuefinder__about-modal__shortcut' },
            ...
            o('div', vc, [
              ...
              o('div', fc, [
                o('div', _c, [
                  ...
                  o('div', gc, [
                    oe(y(s(n)('Escape')) + ' ', 1),
                    i[4] || (i[4] = o('div', null, [o('kbd', null, 'Esc')], -1)),
                  ]),
                  o('div', wc, [
                    ...
                    i[5] ||
                      (i[5] = o(
                        'div',
                        null,
                        [o('kbd', null, 'Ctrl'), oe(' + '), o('kbd', null, 'A')],
                        -1
                      )),
                  ]),
                  o('div', yc, [
                    ...
                    i[6] ||
                      (i[6] = o(
                        'div',
                        null,
                        [o('kbd', null, 'Ctrl'), oe(' + '), o('kbd', null, 'F')],
                        -1
                      )),
                  ]),
                  o('div', bc, [
                    ...
                    i[7] ||
                      (i[7] = o(
                        'div',
                        null,
                        [o('kbd', null, 'Ctrl'), oe(' + '), o('kbd', null, 'E')],
                        -1
                      )),
                  ]),
                  o('div', xc, [
                  ...
                  o('div', kc, [
                    ...
                    i[9] ||
                      (i[9] = o(
                        'div',
                        null,
                        [o('kbd', null, 'Ctrl'), oe(' + '), o('kbd', null, 'Enter')],
                        -1
                      )),
                  ]),
                  o('div', $c, [
                    oe(y(s(n)('Preview')) + ' ', 1),
                    i[10] || (i[10] = o('div', null, [o('kbd', null, 'Space')], -1)),
                  ]),
                ]),
              ]),
            ]),
  ...
  Sc = { class: 'vuefinder__menubar__container' },
  Fc = ['onClick', 'onMouseenter'],
  Dc = { class: 'vuefinder__menubar__label' },
  Ec = ['onMouseenter'],
  Ac = ['onClick'],
  Mc = {
    key: 0,
  ...
  Ic = /* @__PURE__ */ X({
    ...
    setup(t) {
      const e = ee(),
        ...
        c = E(null),
        _ = E(!1),
        v = U(() => window.opener !== null || window.name !== '' || window.history.length <= 1),
        d = U(() => [
          {
            id: 'file',
            label: n('File'),
              ...
              },
              {
                ...
                action: () => {
                  a.value.length === 1 &&
                    ...
                    e?.modal?.open(mn, { items: a.value });
                },
                enabled: () =>
                  a.value.length === 1 &&
                  a.value[0]?.mime_type === 'application/zip' &&
                  e?.features?.includes(ne.UNARCHIVE),
              },
              { type: 'separator' },
              {
                id: 'preview',
                ...
                action: () => {
                  a.value.length === 1 &&
                    a.value[0]?.type !== 'dir' &&
                    e?.modal?.open(It, { storage: l?.path?.get()?.storage, item: a.value[0] });
                },
          ...
          },
          {
            ...
            items: [
              ...
              },
              {
                ...
                action: () => {
                  a.value.length > 0 &&
                    l?.setClipboard('copy', new Set(a.value.map((m) => m.path)));
                },
                enabled: () => a.value.length > 0,
              },
              {
                      ...
                      w(
                        'div',
                        {
                          ...
                          class: j([
                            ...
                            { 'vuefinder__menubar__item--active': c.value === C.id },
                          ]),
                          onClick: ($) => h(C.id),
                          onMouseenter: ($) => b(C.id),
                        },
                                      ...
                                      ($) => (
                                        ...
                                        w(
                                          'div',
                                          {
                                            ...
                                            onClick: ae(
                                              (O) =>
                                                $.type !== 'separator' && $.enabled && $.enabled()
                                                  ? g($.action)
                                                  : null,
                                              ['stop']
                                            ),
...
const qc = { render: Gc },
  Yc = { class: 'vuefinder__toolbar' },
  Qc = { class: 'vuefinder__toolbar__actions' },
  Xc = ['title'],
  Jc = ['title'],
  Zc = ['title'],
  eu = ['title'],
  ...
  nu = ['title'],
  ou = ['title'],
  su = { class: 'vuefinder__toolbar__controls' },
  lu = ['title'],
  iu = { class: 'vuefinder__toolbar__control vuefinder__toolbar__dropdown-container' },
  au = ['title'],
  ru = { class: 'relative' },
  du = {
    key: 0,
  ...
  uu = { class: 'vuefinder__toolbar__dropdown-content' },
  vu = { class: 'vuefinder__toolbar__dropdown-section' },
  fu = { class: 'vuefinder__toolbar__dropdown-label' },
  _u = { class: 'vuefinder__toolbar__dropdown-row' },
  mu = { value: 'name' },
  pu = { value: 'size' },
  hu = { value: 'modified' },
  gu = { value: '' },
  wu = { value: 'asc' },
  yu = { value: 'desc' },
  bu = { class: 'vuefinder__toolbar__dropdown-section' },
  xu = { class: 'vuefinder__toolbar__dropdown-label' },
  ku = { class: 'vuefinder__toolbar__dropdown-options' },
  $u = { class: 'vuefinder__toolbar__dropdown-option' },
  Cu = { class: 'vuefinder__toolbar__option-text' },
  Su = { class: 'vuefinder__toolbar__dropdown-option' },
  Fu = { class: 'vuefinder__toolbar__option-text' },
  Du = { class: 'vuefinder__toolbar__dropdown-option' },
  Eu = { class: 'vuefinder__toolbar__option-text' },
  Au = { class: 'vuefinder__toolbar__dropdown-toggle' },
  Mu = {
    for: 'showHidden',
    class: 'vuefinder__toolbar__toggle-label',
  },
  Tu = { class: 'vuefinder__toolbar__dropdown-reset' },
  Iu = ['title'],
  Ou = ['title'],
  Lu = /* @__PURE__ */ X({
    name: 'VfToolbar',
    ...
    setup(t) {
      ...
      (ue(
        ...
        (g) => {
          ...
          g === 'name'
            ? l.setSort('basename', d.value.sortOrder)
            : g === 'size'
              ? l.setSort('file_size', d.value.sortOrder)
              : g === 'modified' && l.setSort('last_modified', d.value.sortOrder);
        }
      ),
        ue(
          () => d.value.sortOrder,
          (g) => {
            ...
            d.value.sortBy === 'name'
              ? l.setSort('basename', g)
              : d.value.sortBy === 'size'
                ? l.setSort('file_size', g)
                : d.value.sortBy === 'modified' && l.setSort('last_modified', g);
          }
        ),
        ue(
          f,
          (g) => {
            g.active
              ? (g.column === 'basename'
                  ...
                  : g.column === 'file_size'
                    ? (d.value.sortBy = 'size')
                    : g.column === 'last_modified' && (d.value.sortBy = 'modified'),
                (d.value.sortOrder = g.order))
              : (d.value.sortOrder = '');
          },
          { immediate: !0 }
        ),
      ...
      const h = () => i.set('view', r.value.view === 'grid' ? 'list' : 'grid'),
        b = U(() => c.value.kind !== 'all' || !r.value.showHiddenFiles || f.value.active),
        D = () => {
          ((d.value = {
            sortBy: 'name',
            sortOrder: '',
      ...
      return (g, p) => (
        ...
        w('div', Yc, [
          o('div', Qc, [
            s(e).features.includes(s(ne).NEW_FOLDER)
              ? (u(),
                w(
                  'div',
                  {
              ...
              : M('', !0),
            s(e).features.includes(s(ne).NEW_FILE)
              ? (u(),
                w(
                  'div',
                  {
              ...
              : M('', !0),
            s(e).features.includes(s(ne).RENAME)
              ? (u(),
                w(
                  'div',
                  {
                    ...
                    onClick:
                      p[2] ||
                      (p[2] = (m) => s(a).length !== 1 || s(e).modal.open(Tt, { items: s(a) })),
                  },
                  [
                    I(
                      s(Hn),
                      {
                        class: j(
                          s(a).length === 1 ? 'vf-toolbar-icon' : 'vf-toolbar-icon-disabled'
                        ),
                      },
                      null,
                      8,
              ...
              : M('', !0),
            s(e).features.includes(s(ne).DELETE)
              ? (u(),
                w(
                  'div',
                  {
                    ...
                    title: s(n)('Delete'),
                    onClick:
                      p[3] || (p[3] = (m) => !s(a).length || s(e).modal.open(Mt, { items: s(a) })),
                  },
                  [
                    I(
                      s(zn),
              ...
              : M('', !0),
            s(e).features.includes(s(ne).UPLOAD)
              ? (u(),
                w(
                  'div',
                  {
              ...
              : M('', !0),
            s(e).features.includes(s(ne).UNARCHIVE) &&
            s(a).length === 1 &&
            s(a)[0].mime_type === 'application/zip'
              ? (u(),
                w(
                  'div',
                  {
                    ...
                    title: s(n)('Unarchive'),
                    onClick:
                      p[5] || (p[5] = (m) => !s(a).length || s(e).modal.open(mn, { items: s(a) })),
                  },
                  [
                    I(
                      s(vo),
              ...
              : M('', !0),
            s(e).features.includes(s(ne).ARCHIVE)
              ? (u(),
                w(
                  'div',
                  {
                    ...
                    title: s(n)('Archive'),
                    onClick:
                      p[6] || (p[6] = (m) => !s(a).length || s(e).modal.open(pn, { items: s(a) })),
                  },
                  [
                    I(
                      s(fo),
          ...
          o('div', su, [
            s(e).features.includes(s(ne).SEARCH)
              ? (u(),
                w(
                  'div',
                  {
                        ...
                        o('div', _u, [
                          ...
                          me(
                            o(
                              ...
                              {
                                'onUpdate:modelValue':
                                  p[10] || (p[10] = (m) => (d.value.sortOrder = m)),
                                class: 'vuefinder__toolbar__dropdown-select',
                              },
                              [
                            ...
                            me(
                              o(
                                'input',
                                {
                                  ...
                                  value: 'all',
                                  'onUpdate:modelValue':
                                    p[11] || (p[11] = (m) => (d.value.filterKind = m)),
                            ...
                            me(
                              o(
                                'input',
                                {
                                  ...
                                  value: 'files',
                                  'onUpdate:modelValue':
                                    p[12] || (p[12] = (m) => (d.value.filterKind = m)),
                                  class: 'vuefinder__toolbar__radio',
                                },
                                null,
                                512
                            ...
                            me(
                              o(
                                'input',
                                {
                                  ...
                                  value: 'folders',
                                  'onUpdate:modelValue':
                                    p[13] || (p[13] = (m) => (d.value.filterKind = m)),
                                  class: 'vuefinder__toolbar__radio',
                                },
                                null,
                                512
                        ...
                        me(
                          o(
                            'input',
                            {
                              ...
                              id: 'showHidden',
                              'onUpdate:modelValue':
                                p[14] || (p[14] = (m) => (d.value.showHidden = m)),
            ...
            s(e).features.includes(s(ne).FULL_SCREEN)
              ? (u(),
                w(
                  'div',
                  {
...
const lv = { render: sv };
function mt(t, e = []) {
  const n = 'vfDragEnterCounter',
    l = t.fs,
    i = W(l.selectedItems);
  function r(v, d) {
    if (v.isExternalDrag) return;
    (v.preventDefault(),
      l.getDraggedItem() === d.path ||
      !d ||
      d.type !== 'dir' ||
      i.value.some((b) => b.path === d.path || Na(b.path) === d.path)
        ? v.dataTransfer &&
          ((v.dataTransfer.dropEffect = 'none'), (v.dataTransfer.effectAllowed = 'none'))
        : (v.dataTransfer &&
            ((v.dataTransfer.dropEffect = 'copy'), (v.dataTransfer.effectAllowed = 'all')),
          v.currentTarget.classList.add(...e)));
  }
  function a(v) {
    if (v.isExternalDrag) return;
    v.preventDefault();
    const d = v.currentTarget,
      h = Number(d.dataset[n] || 0);
  ...
  function c(v, d) {
    ...
    const b = v.dataTransfer?.getData('items') || '[]',
      g = JSON.parse(b).map((p) => l.sortedFiles.get().find((m) => m.path === p));
...
const iv = { class: 'vuefinder__breadcrumb__container' },
  av = ['title'],
  rv = ['title'],
  dv = ['title'],
  cv = ['title'],
  uv = { class: 'vuefinder__breadcrumb__path-container' },
  vv = { class: 'vuefinder__breadcrumb__list' },
  fv = {
    key: 0,
  ...
  mv = ['title', 'onClick'],
  pv = ['title'],
  hv = { class: 'vuefinder__breadcrumb__path-mode' },
  gv = { class: 'vuefinder__breadcrumb__path-mode-content' },
  wv = ['title'],
  yv = { class: 'vuefinder__breadcrumb__path-text' },
  bv = ['title'],
  xv = ['data-theme'],
  kv = ['onClick'],
  $v = { class: 'vuefinder__breadcrumb__hidden-item-content' },
  Cv = { class: 'vuefinder__breadcrumb__hidden-item-text' },
  Sv = /* @__PURE__ */ X({
    __name: 'Breadcrumb',
    setup(t) {
      const e = ee(),
        ...
        d = E(!1),
        h = E(!1),
        b = U(() => a.value?.breadcrumb ?? []);
      function D(Z, S) {
        return Z.length > S ? [Z.slice(-S), Z.slice(0, -S)] : [Z, []];
      }
      const g = U(() => D(b.value, v.value)[0]),
        p = U(() => D(b.value, v.value)[1]);
      ue(_, () => {
        if (!c.value) return;
        ...
        ((v.value = F),
          Re(() => {
            for (let H = Z.length - 1; H >= 0; H--) {
              const Y = Z[H];
              if (S + Y.offsetWidth > _.value - 40) break;
      ...
      const O = () => {
        ...
        le = async () => {
          (await ut(a.value?.path || ''),
            e.emitter.emit('vf-toast-push', { label: n('Path copied to clipboard') }));
        },
        _e = () => {
          h.value = !1;
        };
        ...
        w('div', iv, [
              ...
              I(
                s(Zu),
                {
                  ...
                  class: j([
                    ...
                    s(r).showTreeView ? 'vuefinder__breadcrumb__toggle-tree--active' : '',
                  ]),
                },
                null,
                8,
          ...
          o(
            ...
            [
              I(
                s(Hu),
                Te(We(b.value.length ? s(C).events($()) : {}), {
                  onClick: z,
                  class: b.value.length
            ...
            o(
              ...
              [
                o('div', null, [
                  I(
                    s(Gu),
                    Te({ class: 'vuefinder__breadcrumb__home-icon' }, We(s(C).events($(-1))), {
                      onClick:
                        S[1] ||
                        (S[1] = ae((x) => s(e).adapter.open(s(a).storage + '://'), ['stop'])),
                    }),
                    null,
                    16
                  ),
                ...
                s(i).get('loadingIndicator') === 'circular' && s(f)
                  ? (u(), V(s(Rt), { key: 0 }))
                  : M('', !0),
                o(
                  'span',
...
function Fv(t, e) {
  const {
    ...
    c = t,
    _ = () => (typeof i == 'number' ? i : i.value),
    v = E(0),
    d = E(6),
    h = E(600);
  let b = null;
  const D = U(() => Math.ceil(c.value.length / d.value)),
    g = U(() => D.value * _()),
    p = U(() => {
      const R = _(),
        q = Math.max(0, Math.floor(v.value / R) - r),
        re = Math.min(D.value, Math.ceil((v.value + h.value) / R) + r);
...
function Dv(t) {
  const {
    ...
    f = Math.floor(Math.random() * 2 ** 32).toString(),
    c = ee(),
    _ = c.fs,
    v = W(_.selectedKeys),
    d = W(_.sortedFiles),
    h = E(/* @__PURE__ */ new Set()),
    b = E(!1),
    D = E(!1),
    g = E(null),
    p = (S) => S.map((x) => x.getAttribute('data-key')).filter((x) => !!x),
    m = (S) => {
    ...
    C = (S) => {
      ...
      if (!x) return !1;
      const F = c.selectionFilterType,
        A = c.selectionFilterMimeIncludes;
      return (F === 'files' && x.type === 'dir') || (F === 'dirs' && x.type === 'file')
        ? !1
        : A && Array.isArray(A) && A.length > 0
          ? x.type === 'dir'
            ? !0
            : x.mime_type
              ? A.some((H) => x.mime_type?.startsWith(H))
              : !1
          : !0;
    },
    $ = (S) => {
      ...
      const F = Array.from(S).map((ce) => {
        ...
        A = Math.min(...F.map((ce) => ce.row)),
        H = Math.max(...F.map((ce) => ce.row)),
        Y = Math.min(...F.map((ce) => ce.col)),
        he = Math.max(...F.map((ce) => ce.col));
      return { minRow: A, maxRow: H, minCol: Y, maxCol: he };
    },
    O = (S) => {
      if (c.selectionMode === 'single') return !1;
      ((b.value = !1),
        !S.event?.metaKey && !S.event?.ctrlKey && (D.value = !0),
        S.selection.resolveSelectables(),
        m(S),
        k(S));
    },
    z = E(0),
    ...
    G = ({ event: S, selection: x }) => {
      ...
      if ((F && (F.dataset.theme = c.theme.current), c.selectionMode === 'single')) return;
      const A = S;
      A && 'type' in A && A.type === 'touchend' && A.preventDefault();
      const H = S;
      if (
        (!H?.ctrlKey && !H?.metaKey && (_.clearSelection(), x.clearSelection(!0, !0)),
        h.value.clear(),
        i.value)
      ) {
        ...
        if (Y) {
          const he = Y.getBoundingClientRect(),
            ce = L(S);
          if (ce) {
            const Be = ce.y - he.top + Y.scrollTop,
              Ue = ce.x - he.left,
              Ze = Math.floor(Be / r.value),
              st = Math.floor(Ue / a);
    ...
    R = (S) => {
      ...
      const x = S.selection,
        F = p(S.store.changed.added),
        A = p(S.store.changed.removed);
      ((D.value = !1),
        (b.value = !0),
        F.forEach((H) => {
          v.value &&
            !v.value.has(H) &&
            C(H) &&
            (h.value.add(H), _.select(H, c.selectionMode || 'multiple'));
        }),
        A.forEach((H) => {
          (document.querySelector(`[data-key="${H}"]`) &&
            d.value?.find((he) => l(he) === H) &&
            h.value.delete(H),
            _.deselect(H));
        }),
        x.resolveSelectables(),
        k(S));
    },
    q = () => {
      h.value.clear();
    ...
    re = (S) => {
      if (S.event && g.value && h.value.size > 0) {
        ...
        if (F.length > 0) {
          ...
          n(d.value || [], H.minRow, H.maxRow, H.minCol, H.maxCol).forEach((Y) => {
            const he = l(Y);
            document.querySelector(`[data-key="${he}"]`) ||
              _.select(he, c.selectionMode || 'multiple');
          });
        }
      }
    },
    ...
    _e = () => {
      i.value &&
        (Array.from(v.value ?? /* @__PURE__ */ new Set()).forEach((x) => {
          C(x) || _.deselect(x);
        }),
        le(),
        J());
    ...
    Z = (S) => {
      ...
      !h.value.size &&
        ...
        !x?.ctrlKey &&
        !x?.metaKey &&
        (_.clearSelection(), i.value?.clearSelection());
    };
  return (
    ve(() => {
      const S = (x) => {
...
const Vv = { render: Rv },
  Pv = { class: 'vuefinder__drag-item__container' },
  Bv = { class: 'vuefinder__drag-item__count' },
  zv = /* @__PURE__ */ X({
    __name: 'DragItem',
    props: {
      count: {},
        ...
        w(
          ...
          [
            De(r.$slots, 'icon', rt(dt(i)), () => [
              ...
              t.ext && t.item.type !== 'dir' && t.item.extension
                ? (u(), w('div', Hv, y(t.item.extension.substring(0, 3)), 1))
                : M('', !0),
            ]),
          ],
          2
...
const En = { render: Uv },
  Kv = ['data-key', 'data-row', 'data-col', 'draggable'],
  Wv = { key: 0 },
  jv = { class: 'vuefinder__explorer__item-grid-content' },
  Gv = ['data-src', 'alt'],
  qv = { class: 'vuefinder__explorer__item-title' },
  Yv = {
    key: 1,
    class: 'vuefinder__explorer__item-list-content',
  },
  Qv = { class: 'vuefinder__explorer__item-list-name' },
  Xv = { class: 'vuefinder__explorer__item-list-icon' },
  Jv = { class: 'vuefinder__explorer__item-name' },
  Zv = {
    key: 0,
    ...
    setup(t, { emit: e }) {
      const n = t,
        ...
        f = U(() => {
          ...
          return !k || k === 'both'
            ? !0
            : (k === 'files' && n.item.type === 'file') || (k === 'dirs' && n.item.type === 'dir');
        }),
        c = U(() => {
          const k = i.selectionFilterMimeIncludes;
          return !k || !k.length || n.item.type === 'dir'
            ? !0
            : n.item.mime_type
              ? k.some((C) => n.item.mime_type?.startsWith(C))
              : !1;
        }),
        _ = U(() => f.value && c.value),
        v = U(() => [
      ...
      const g = () => {
        ...
        m = (k) => {
          ...
          else return ((D = !1), l('dblclick', k), h && clearTimeout(h), !1);
              ...
              ? (u(),
                w('div', Wv, [
                  s(r).isReadOnly(t.item)
                    ...
                    : M('', !0),
                  o('div', jv, [
                    (t.item.mime_type ?? '').startsWith('image') && t.showThumbnails
                      ? (u(),
                        w(
                          'img',
                          {
                ...
                w('div', Yv, [
                  ...
                  t.showPath
                    ...
                    : (u(),
                      w('div', ef, [
                        t.item.file_size
                          ? (u(), w('div', tf, y(s(i).filesize(t.item.file_size)), 1))
                          : M('', !0),
                      ])),
                  !t.showPath && t.item.last_modified
                    ? (u(),
                      w('div', nf, y(new Date(t.item.last_modified * 1e3).toLocaleString()), 1))
                    : M('', !0),
                ])),
            s(a)
              .get('pinnedFolders')
              .find(($) => $.path === t.item.path)
              ? (u(),
                V(s(tn), {
                  key: 2,
                  ...
                  fe(
                    ...
                    (_, v) => (
                      ...
                      V(
                        ...
                        {
                          icon: Q((d) => [De(f.$slots, 'icon', Te({ ref_for: !0 }, d))]),
                          _: 3,
                        },
                        16,
                        [
  ...
  af = /* @__PURE__ */ X({
    ...
    setup(t) {
      const e = ee(),
        ...
        f = (c) => {
          let _ = i.value.findIndex((v) => v.id === c);
          _ !== -1 && a(_);
        };
      return (
  ...
  rf = { class: 'vuefinder__explorer__container' },
  df = {
    ref: 'customScrollBar',
  ...
  vf = /* @__PURE__ */ X({
    ...
    setup(t) {
      const e = t,
        n = ee(),
        l = mt(n, ['vuefinder__drag-over']),
        i = Ke('dragImage'),
        r = Mn(null),
        a = Ke('scrollContainer'),
        f = Ke('scrollContent'),
        c = n.fs,
        _ = n.config,
        v = W(_.state),
      ...
      let p = null;
      const m = E(null),
        k = Ke('customScrollBar'),
        C = Ke('customScrollBarContainer'),
        $ = U(() => {
          const B = v.value.view,
            te = v.value.compactListView;
      ...
      (ve(() => {
        if (
          (Z(),
          r.value &&
            r.value.on('beforestart', ({ event: B }) => {
              const te = B?.target === f.value;
              if (!B?.metaKey && !B?.ctrlKey && !B?.altKey && !te) return !1;
            }),
          a.value &&
            (p = new Ln({
        ...
        xo(() => {
          if ((p && p.update(), m.value && k.value && a.value)) {
            const te = a.value.scrollHeight > a.value.clientHeight,
              se = k.value;
      ...
      const he = (B) => {
          const te = B.target?.closest('.file-item-' + le),
            se = B;
          if (te) {
            const ie = String(te.getAttribute('data-key')),
              T = h.value?.find(($e) => $e.path === ie),
              P = n.selectionFilterType,
              N = n.selectionFilterMimeIncludes,
              K =
                !P ||
                P === 'both' ||
                (P === 'files' && T?.type === 'file') ||
            ...
            if (
              (N &&
                ...
                (T?.type === 'dir'
                  ? (ge = !0)
                  : T?.mime_type
                    ? (ge = N.some(($e) => (T?.mime_type).startsWith($e)))
                    : (ge = !1)),
              !K || !ge)
            )
              return;
            ...
            (!se?.ctrlKey &&
              !se?.metaKey &&
              (B.type !== 'touchstart' || !c.isSelected(ie)) &&
              (c.clearSelection(), r.value?.clearSelection(!0, !0)),
              r.value?.resolveSelectables(),
              B.type === 'touchstart' && c.isSelected(ie)
                ? c.select(ie, ke)
                : c.toggleSelect(ie, ke));
          }
        ...
        ce = (B) => {
          ...
          const te = n.contextMenuItems?.find((se) =>
            se.show(n, {
              items: [B],
              target: B,
              searchQuery: '',
        ...
        Be = (B) => {
          ...
          if (!se) return;
          const ie = h.value?.find((ge) => ge.path === se),
            T = n.selectionFilterType,
            P = n.selectionFilterMimeIncludes,
            N =
              !T ||
              T === 'both' ||
              (T === 'files' && ie?.type === 'file') ||
              (T === 'dirs' && ie?.type === 'dir');
          let K = !0;
          (P &&
            Array.isArray(P) &&
            ...
            (ie?.type === 'dir'
              ? (K = !0)
              : ie?.mime_type
                ? (K = P.some((ge) => (ie?.mime_type).startsWith(ge)))
                : (K = !1)),
            !(!N || !K) && ie && ce(ie));
        },
        Ue = () => {
        ...
        Ze = (B) => {
          ...
          const te = B.target?.closest('.file-item-' + le);
          if (te) {
            const se = String(te.getAttribute('data-key')),
              ie = h.value?.find((ge) => ge.path === se),
              T = n.selectionFilterType,
              P = n.selectionFilterMimeIncludes,
              N =
                !T ||
            ...
            if (
              ...
              return;
            (b.value?.has(se) || (c.clearSelection(), c.select(se)),
              n.emitter.emit('vf-contextmenu-show', { event: B, items: Ue(), target: ie }));
          }
        },
        st = (B) => {
        ...
        pt = (B) => {
          if (B.altKey || B.ctrlKey || B.metaKey) return (B.preventDefault(), !1);
          _e.value = !0;
          const te = B.target?.closest('.file-item-' + le);
          if (((A.value = te ? String(te.dataset.key) : null), B.dataTransfer && A.value)) {
            (B.dataTransfer.setDragImage(i.value, 0, 15),
              (B.dataTransfer.effectAllowed = 'all'),
                        ...
                        fe(
                          ...
                          (se) => (
                            ...
                            V(
                              ...
                              {
                                icon: Q((ie) => [De(B.$slots, 'icon', Te({ ref_for: !0 }, ie))]),
                                _: 3,
                              },
                              8,
                              [
  ...
  mf = /* @__PURE__ */ X({
    ...
    setup(t) {
      ...
      (e.emitter.on('vf-contextmenu-show', (c) => {
        ...
        ((i.items = (e.contextMenuItems || []).filter((h) =>
          h.show(e, {
            items: v,
            target: d,
          })
          ...
          d
            ? v.length > 1 && v.some((h) => h.path === d.path)
      ...
      const f = (c) => {
        ...
        ((i.active = !0),
          Re(() => {
            ...
            let g = D?.height ?? 0,
              p = D?.width ?? 0;
            ((h = d && d.right - c.pageX + window.scrollX < p ? h - p : h),
              (b = d && d.bottom - c.pageY + window.scrollY < g ? b - g : b),
              (i.positions = {
                left: String(h) + 'px',
                top: String(b) + 'px',
              }));
                          ...
                          ? (u(),
                            w(
                              'a',
                              {
                                ...
                                download: r(v),
                                onClick:
                                  _[0] || (_[0] = (d) => s(e).emitter.emit('vf-contextmenu-hide')),
                              },
  ...
  pf = { class: 'vuefinder__status-bar__wrapper' },
  hf = { class: 'vuefinder__status-bar__storage' },
  gf = ['title'],
  wf = { class: 'vuefinder__status-bar__storage-icon' },
  yf = ['value'],
  bf = ['value'],
  xf = { class: 'vuefinder__status-bar__info space-x-2' },
  kf = { key: 0 },
  $f = { key: 1 },
  Cf = {
    key: 0,
  ...
  Sf = { class: 'vuefinder__status-bar__actions' },
  Ff = /* @__PURE__ */ X({
    __name: 'Statusbar',
    setup(t) {
      const e = ee(),
        ...
        d = U(() =>
          !c.value || c.value.length === 0 ? 0 : c.value.reduce((p, m) => p + (m.file_size || 0), 0)
        ),
        h = U(() => f.value),
        b = U(() => i.value),
        D = U(() => a.value || 0),
      ...
      return (p, m) => (
        ...
        w('div', pf, [
          o('div', hf, [
            ...
            o('div', xf, [
              D.value === 0
                ? (u(), w('span', kf, y(b.value.length) + ' ' + y(s(n)('items')), 1))
                : (u(),
                  w('span', $f, [
...
const Mf = { class: 'vuefinder__folder-loader-indicator' },
  Tf = {
    key: 1,
  ...
  If = { key: 0 },
  Of = { class: 'vuefinder__treesubfolderlist__no-folders' },
  Lf = { class: 'vuefinder__treesubfolderlist__item-content' },
  Rf = ['onClick'],
  Vf = ['title', 'onDblclick', 'onClick'],
  Pf = { class: 'vuefinder__treesubfolderlist__item-icon' },
  Bf = { class: 'vuefinder__treesubfolderlist__subfolder' },
  zf = /* @__PURE__ */ X({
        ...
        return (
          ...
          w(
            ...
            [
              _.value.length
                ? M('', !0)
                : (u(), w('li', If, [o('div', Of, y(s(r)('No folders')), 1)])),
              (u(!0),
              w(
                de,
                null,
  ...
  Hf = /* @__PURE__ */ X({
    ...
    setup(t) {
      const e = ee(),
        ...
        l = E(!1),
        i = t,
        r = mt(e, ['vuefinder__drag-over']),
        a = W(n.path),
        f = U(() => i.storage === a.value?.storage),
        c = {
          storage: i.storage,
          path: i.storage + '://',
          dir: i.storage + '://',
          ...
          [
            o(
              ...
              [
                o(
                  'div',
                  Te(We(s(r).events(c), !0), {
                    class: [
                      'vuefinder__treestorageitem__info',
                      f.value ? 'vuefinder__treestorageitem__info--active' : '',
                    ],
                  }),
                  [
                    o(
                      'div',
                      {
                        class: j([
                          ...
                          f.value ? 'vuefinder__treestorageitem__icon--active' : '',
                        ]),
                      },
                      [I(s(nn))],
                      2
  ...
  Nf = { class: 'vuefinder__folder-indicator' },
  Uf = { class: 'vuefinder__folder-indicator--icon' },
  Kf = /* @__PURE__ */ X({
  ...
  Wf = { class: 'vuefinder__treeview__header' },
  jf = { class: 'vuefinder__treeview__pinned-label' },
  Gf = { class: 'vuefinder__treeview__pin-text text-nowrap' },
  qf = {
    key: 0,
  ...
  Xf = ['onClick'],
  Jf = { key: 0 },
  Zf = { class: 'vuefinder__treeview__no-pinned' },
  e_ = /* @__PURE__ */ X({
    __name: 'TreeView',
    setup(t) {
      const e = ee(),
        { t: n } = e.i18n,
        { getStore: l, setStore: i } = e.storage,
        r = e.fs,
        a = e.config,
        f = W(a.state),
        c = W(r.sortedFiles),
        ...
        v = U(() => _.value || []),
        d = W(r.path),
        h = mt(e, ['vuefinder__drag-over']),
        b = E(190),
        D = E(l('pinned-folders-opened', !0));
      ue(D, (k) => i('pinned-folders-opened', k));
      const g = (k) => {
          const C = a.get('pinnedFolders');
          a.set(
            'pinnedFolders',
            C.filter(($) => $.path !== k.path)
          );
        },
        p = (k) => {
          const C = k.clientX,
            $ = k.target.parentElement;
          if (!$) return;
          const O = $.getBoundingClientRect().width;
          ($.classList.remove('transition-[width]'), $.classList.add('transition-none'));
          const z = (G) => {
              ((b.value = O + G.clientX - C),
                b.value < 50 && ((b.value = 0), a.set('showTreeView', !1)),
                b.value > 50 && a.set('showTreeView', !0));
            },
            L = () => {
              const G = $.getBoundingClientRect();
              ((b.value = G.width),
                $.classList.add('transition-[width]'),
            ...
            [
              o(
                'div',
                {
                  ...
                  class: j([
                    ...
                    s(f).showTreeView ? 'vuefinder__treeview__backdrop' : 'hidden',
                  ]),
                },
                null,
                2
              ...
              o(
                'div',
                {
                  style: He(
                    s(f).showTreeView
                      ? 'min-width:100px;max-width:75%; width: ' + b.value + 'px'
                      : 'width: 0'
                  ),
                        ...
                        D.value
                          ? (u(),
                            w('ul', qf, [
                              ...
                              s(f).pinnedFolders.length
                                ? M('', !0)
                                : (u(),
                                  w('li', Jf, [o('div', Zf, y(s(n)('No folders pinned')), 1)])),
                            ]))
                          : M('', !0),
                      ]),
                      (u(!0),
...
function t_(t) {
  return t.items.length > 1 && t.items.some((e) => e.path === t.target?.path)
    ? 'many'
    : t.target
      ? 'one'
      : 'none';
...
function we(t) {
  ...
  return (n, l) =>
    !(
      e.needsSearchQuery !== !!l.searchQuery ||
      (e.target !== void 0 && e.target !== t_(l)) ||
      (e.targetType !== void 0 && e.targetType !== l.target?.type) ||
      (e.mimeType !== void 0 && e.mimeType !== l.target?.mime_type) ||
      (e.feature !== void 0 && !n.features.includes(e.feature))
    );
}
function it(...t) {
...
const n_ = [
    ...
    },
    {
      ...
      action: (t, e) => {
        const n = t.config,
          l = n.get('pinnedFolders'),
          i = l.concat(e.filter((r) => l.findIndex((a) => a.path === r.path) === -1));
    ...
    },
    {
      ...
      action: (t, e) => {
        ...
        n.set(
          'pinnedFolders',
          l.filter((i) => !e.find((r) => r.path === i.path))
        );
      },
      show: at(
        we({ target: 'one', targetType: 'dir' }),
    ...
    },
    {
      ...
      show: at(we({ target: 'one', feature: ne.PREVIEW }), (t, e) => e.target?.type !== 'dir'),
    },
    {
      id: be.download,
      link: (t, e) => {
        if (e[0]) return t.adapter.getDownloadUrl(e[0]);
      },
      title: ({ t }) => t('Download'),
      action: () => {},
      show: at(we({ target: 'one', feature: ne.DOWNLOAD }), (t, e) => e.target?.type !== 'dir'),
    },
    {
      id: be.rename,
      title: ({ t }) => t('Rename'),
    ...
    },
    {
      ...
      show: it(we({ target: 'one', feature: ne.MOVE }), we({ target: 'many', feature: ne.MOVE })),
    },
    {
      id: be.copy,
      title: ({ t }) => t('Copy'),
      ...
      show: it(we({ target: 'one', feature: ne.COPY }), we({ target: 'many', feature: ne.COPY })),
    },
    {
      id: be.paste,
      title: ({ t }) => t('Paste'),
  ...
  l_ = { class: 'vuefinder__external-drop-message' },
  i_ = { class: 'vuefinder__main__content' },
  a_ = /* @__PURE__ */ X({
    __name: 'VueFinder',
    props: {
      id: { default: 'vf' },
    ...
    setup(t, { emit: e }) {
      ...
      function b(g) {
        (a.setPath(g.dirname),
          r.get('persist') && r.set('path', g.dirname),
          a.setReadOnly(g.read_only ?? !1),
          i.modal.close(),
          a.setFiles(g.files),
          a.clearSelection(),
          a.setSelectedCount(0),
          a.setStorages(g.storages));
      }
        ...
        w(
          'div',
          {
            ...
            class: j([
              ...
              { 'vuefinder--dragging-external': s(c) },
            ]),
            'data-theme': s(i).theme.current,
            onDragenter:
              p[2] ||
                  ...
                  [
                      ...
                      ? (u(),
                        w('div', s_, [
                          o(
                            'div',
                            l_,
                            y(s(i).i18n.t('Drag and drop the files/folders to here.')),
                            1
                          ),
                        ]))
                      : M('', !0),
                    ...
                    o('div', i_, [
                      ...
                      I(
                        ...
                        {
                          icon: Q((m) => [De(g.$slots, 'icon', rt(dt(m)))]),
                          _: 3,
                        },
                        8,
                    ...
                    I(Ff, null, {
                      actions: Q((m) => [De(g.$slots, 'status-bar', rt(dt(m)))]),
                      _: 3,
                    }),
                  ],
                  34
